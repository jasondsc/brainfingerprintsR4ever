...
# permutation test on self similarity
...
# scatter plot of differentiability effect with age
...
# linear regression model to see if sex plays a role in age-differentiation relationship
...
```{r bootstrap fingerprinting accuracy effects}
...
```{r equate groups based on artifacts 2nd try}
...
# test whether subsamople has similar cranial volume
kids= volume[demo$Group == "below 12 years old" & log10(rowMeans(artifacts)) < -0.6 & volume > 210000  ]
adults=volume[demo$Group == "18+ years old"  & volume < 270000 & log10(rowMeans(artifacts)) > -1.5] 
RVAideMemoire::perm.t.test(kids ,adults, nperm=1000)
BayesFactor::ttestBF(kids[!is.na(kids)] ,adults)
# t = 1.1215, p-value = 0.2577 BF = 0.3032815
plottt=data.frame(cranialVolume= c(kids, adults), GROUP= c(rep('1 below 12 years old', length(kids)), rep('18+ years old', length(adults))))
...
ggsave('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_figures/REVIEW_BOXPLOT_cranialVolume.pdf', device = "pdf")

# test head motion effects
kids= log10(rowMeans(artifacts[demo$Group == "below 12 years old" & log10(rowMeans(artifacts)) < -0.6 & volume < 270000,]))
adults=log10(rowMeans(artifacts[demo$Group == "18+ years old"  & volume < 270000 & log10(rowMeans(artifacts)) > -1.5,] ))
RVAideMemoire::perm.t.test(kids ,adults, nperm=1000)
BayesFactor::ttestBF(kids[!is.na(kids)] ,adults)
#t = 1.5471, p-value = 0.1359 BF 0.5086485

plottt=data.frame(artifacts= c(kids, adults), GROUP= c(rep('1 below 12 years old', length(kids)), rep('18+ years old', length(adults))))
...
ggsave('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_figures/REVIEW_BOXPLOT_artifacts.pdf', device = "pdf")



# cross corr effect 
indexK= demo$Group == "below 12 years old" & log10(rowMeans(artifacts)) < -0.6 & volume > 210000
indexA= demo$Group == "18+ years old" & volume < 270000 & log10(rowMeans(artifacts)) > -1.5#& rowMeans(headdist) < 0.135
...
adults=new_identifiability[57:201]
RVAideMemoire::perm.t.test(kids ,adults, nperm=1000)
BayesFactor::ttestBF(kids, adults)
# t = -4.5028, p-value = 0.001998 BF= 1481.944
...
ggsave('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_figures/REVIEW_BOXPLOT_differentiability_cohort22.pdf', device = "pdf")
...
ggplot(data4plot, aes(X, Y, fill= values)) + 
  geom_tile() + scale_y_reverse()+  theme_void() + colorspace::scale_fill_continuous_sequential(palette= 'Batlow', rev= FALSE, limits= c(0.75, 1)) + geom_hline(yintercept = 57, color= "white") + geom_vline(xintercept = 57, color= "white")

ggsave('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_figures/REVIEW_Corr_matrix_all_people_new_cohort.pdf', device = "pdf", width = 5, height = 5)

ggsave('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_figures/REVIEW_Corr_matrix_all_people_new_cohort.jpg', device = "jpg", width = 5, height = 5)


# ICC maps 
# cross corr effect 
indexK= demo$Group == "below 12 years old" & log10(rowMeans(artifacts)) < -0.6 & volume > 210000
indexA= demo$Group == "18+ years old" & volume < 270000 #& rowMeans(headdist) < 0.135
...
icc_diff=read.csv('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_output/ICC_youngkids_analysis_matched.csv')

icc_diff=icc_diff[,-1]

icc_sickkids=read.csv('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_output/ICC_youngkids_analysis.csv')
...
icc_diff=read.csv('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_output/ICC_CORRSPEC_youngkids_analysis.csv')

icc_diff=icc_diff[,-1]

icc_sickkids=read.csv('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_output/ICC_CORRSPEC_youngkids_analysis_matched.csv')

icc_sickkids=icc_sickkids[,-1]


XkidsOrig=data.frame(theta= rowMeans(icc_sickkids[,1:8]),alpha= rowMeans(icc_sickkids[,9:18]),beta= rowMeans(icc_sickkids[,19:52]), gamma= rowMeans(icc_sickkids[,53:93]))

Xkids=data.frame(theta= rowMeans(icc_diff[,1:8]),alpha= rowMeans(icc_diff[,9:18]),beta= rowMeans(icc_diff[,19:52]), gamma= rowMeans(icc_diff[,53:93]))
...
someData6$ICC[someData6$ICC >0.8]=0.8
...
ggplot(someData6) +
  geom_brain(atlas = desterieux, 
             position = position_brain(hemi ~ side),
             aes(fill = ICC)) + 
  colorspace::scale_fill_continuous_sequential(palette= 'Batlow', rev= FALSE, limits= c(0.45, 0.8))+ 
  theme_void() + scale_color_manual('white')

ggsave('C:/Users/J Da Silva Castanhei/Documents/SickKidz_fingerprinting/R_figures/2024brain_map_ICC_young_matched.pdf', device = "pdf")
...
permuted_index=permuted_index+1
#gradient offset
orig=cor.test(rowMeans(Xkids), rowMeans(XkidsOrig))
permuted_corr=c()
for (i in 1:1000){
  
  cor_temp=cor.test(rowMeans(Xkids), rowMeans(XkidsOrig)[permuted_index[,i]])
  permuted_corr= c(permuted_corr, cor_temp$estimate)
  
}
sum(orig$estimate < permuted_corr)+1/1001 # p 0.00
